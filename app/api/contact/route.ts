...
const SMTP_CONFIG = {
  ...
  secure: true, // 465 = SSL
  auth: {
    user: "contact@moha-plombier-chauffagiste.fr",
    pass: "MohaParis2025",
  },
  from: 'Moha Plombier Chauffagiste <contact@moha-plombier-chauffagiste.fr>',
  to: "contact@moha-plombier-chauffagiste.fr",
};
...
export async function POST(request: Request) {
  ...
  // Transporter Nodemailer configuré en dur pour IONOS
  const transporter = nodemailer.createTransport({
    ...
    tls: {
      rejectUnauthorized: false, // IONOS peut être un peu chiant sur les certifs
    },
  });
